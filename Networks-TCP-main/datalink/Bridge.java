...
/**
 * Layer-2 Bridge connecting two network segments.
 *
 * A bridge has exactly two ports (segment A and segment B).
 * It learns MACs from both sides and selectively forwards or filters frames.
 * Frames destined to the same segment are filtered (not forwarded).
 */
public class Bridge {
    ...
    public void processFrame(Frame frame, DLLNode sender) {
        ...
        if (senderSide == null) {
            System.out.printf("  [BRIDGE %s] ⚠️  Sender %s not connected to this bridge%n", name, sender.name);
            return;
        }
